...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Namaste {he(user.first_name)}! 🙏\n\n"
        f"Admin sawaal bhejega tab aapko notification milega.\n"
        f"Taiyaar rahiye! ✊",
        parse_mode="HTML"
    )
...
async def ask_question(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not ctx.args:
        await update.message.reply_text(
            "Usage: <code>/ask Sawaal likhein</code>", parse_mode="HTML"
        )
        return
    ...
    if not db["students"]:
        await update.message.reply_text("⚠️ Koi student registered nahi hai.")
        return

    webapp_button = InlineKeyboardMarkup([[
        InlineKeyboardButton("📝 Jawab Dijiye", web_app=WebAppInfo(url=WEBAPP_URL))
    ]])
        ...
        try:
            await ctx.bot.send_message(
                ...
                text=(
                    f"📢 <b>Naya Sawaal!</b>\n\n"
                    f"❓ {he(question)}\n\n"
                    f"Neeche button dabake apna jawab dijiye 👇\n"
                    f"<i>(Text, Video ya Audio — aap choose kar sakte hain)</i>"
                ),
                parse_mode="HTML",
...
async def handle_rating(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if str(student_id) in db.get("rated", {}):
        await query.answer("Yeh jawab already rate ho chuka hai! ✅")
        return
    ...
    score_messages = {
        1: "Mehnat aur practice ki zaroorat hai. Himmat mat haaro! 💪",
        2: "Thodi aur preparation chahiye. Keep going! 📚",
        3: "Achha jawab! Aur improvement ki gunjaish hai. 👍",
        4: "Bahut achha jawab! Sahi raaste par hain. 🌟",
        5: "Shaandaar! Ekdum perfect jawab! 🏆"
    }
...
async def reset_question(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    save_db(db)
    await update.message.reply_text(
        "🔄 Question reset ho gaya. Naya sawaal bhejne ke liye /ask use karein."
    )
...
async def list_students(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    db = load_db()
    if not db["students"]:
        await update.message.reply_text("Koi student registered nahi hai abhi.")
        return
...
async def help_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id == ADMIN_ID:
        text = (
            "🛠 <b>Admin Commands:</b>\n\n"
            "<code>/ask Sawaal likhein</code> — Question broadcast karein\n"
            "<code>/students</code> — Registered students list\n"
            "<code>/reset</code> — Question + submissions reset karein\n"
            "<code>/help</code> — Yeh message"
        )
    else:
        text = (
            "📖 <b>Help:</b>\n\n"
            "• Admin ke question ka wait karein\n"
            "• '📝 Jawab Dijiye' button dabao\n"
            "• Text, Video ya Audio mein jawab dijiye\n"
            "• Score aapke Telegram DM mein aayega"
        )
